    ...
    try:
        with open(filename, newline='') as csvfile:
            ...
            for row in spamreader:
                for i in range(1, 3): #was 1 to 5
                    
                    if row[i] not in ips:
                        print(f"row[i] isssss: {row[i]}")
                        ips[row[i]] = 1
                    else:
                        ips[row[i]] += 1
            return ips
...
def build_ip_profiles(website):
    ...
    if not folder:
        ...
        for file in csv_files:
            print(f"+++++++++currently within this file: {file}++++++++++++++++++")
            trace_ips = get_trace_ips(file)
            # print(" ............... length of trace_ips is:  ", len(trace_ips))
            for key in trace_ips:
                # print(f"################### the key in trace_ips({file}) is:  {key} #############")
